...
Modified to take hyperparameter kwargs in configuration
"""
...
from stardist3dcustom import StarDist3DCustom
...
def configure(Y: List[ndarray], n_channel: int, up_limit: int = UP_LIMIT, model_name: str = 'stardist',
              basedir: str = STARDIST_MODELS, **kwargs):
    ...
    if not kwargs.get('train_batch_size'): kwargs['train_batch_size'] = 2
    if not kwargs.get('unet_n_depth'): kwargs['unet_n_depth'] = 2
    if not kwargs.get('unet_pool'): kwargs['unet_pool'] = 2, 2, 2
    unet_n_depth = kwargs['unet_n_depth']
    unet_pool = kwargs['unet_pool']
